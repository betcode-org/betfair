...
def list_event_types(api, params=None, session=None, parsed=True):
    if not params:
        params = {'filter': {}}
    (response, raw_response, sent) = BettingRequest(api, 'SportsAPING/v1.0/listEventTypes', params).call(session)
    if response:
...
def list_competitions(api, params=None, session=None, parsed=True):
    if not params:
        params = {'filter': {}}
    (response, raw_response, sent) = BettingRequest(api, 'SportsAPING/v1.0/listCompetitions', params).call(session)
    if response:
        if not apierrorhandling.api_betting_error_handling(response, params):
            return
        if parsed:
...
def list_time_ranges(api, params=None, session=None, parsed=True):
    if not params:
        params = {'filter': {},
                  'granularity': 'DAYS'}
    (response, raw_response, sent) = BettingRequest(api, 'SportsAPING/v1.0/listTimeRanges', params).call(session)
    if response:
        if not apierrorhandling.api_betting_error_handling(response, params):
            return
        if parsed:
...
def list_events(api, params=None, session=None, parsed=True):
    if not params:
        params = {'filter': {}}
    (response, raw_response, sent) = BettingRequest(api, 'SportsAPING/v1.0/listEvents', params).call(session)
    if response:
        if not apierrorhandling.api_betting_error_handling(response, params):
            return
        if parsed:
...
def list_market_types(api, params=None, session=None, parsed=True):
    if not params:
        params = {'filter': {}}
    (response, raw_response, sent) = BettingRequest(api, 'SportsAPING/v1.0/listMarketTypes', params).call(session)
    if response:
        if not apierrorhandling.api_betting_error_handling(response, params):
            return
        if parsed:
...
def list_countries(api, params=None, session=None, parsed=True):
    if not params:
        params = {'filter': {}}
    (response, raw_response, sent) = BettingRequest(api, 'SportsAPING/v1.0/listCountries', params).call(session)
    if response:
        if not apierrorhandling.api_betting_error_handling(response, params):
            return
        if parsed:
...
def list_venues(api, params=None, session=None, parsed=True):
    if not params:
        params = {'filter': {}}
    (response, raw_response, sent) = BettingRequest(api, 'SportsAPING/v1.0/listVenues', params).call(session)
    if response:
        if not apierrorhandling.api_betting_error_handling(response, params):
            return
        if parsed:
...
def list_market_catalogue(api, params=None, session=None, parsed=True):
    if not params:
        params = {'filter': {},
                  'maxResults': '1'}
    (response, raw_response, sent) = BettingRequest(api, 'SportsAPING/v1.0/listMarketCatalogue', params).call(session)
    if response:
        if not apierrorhandling.api_betting_error_handling(response, params):
            return
        if parsed:
...
def list_market_book(api, params=None, session=None, parsed=True):
    ...
    (response, raw_response, sent) = BettingRequest(api, 'SportsAPING/v1.0/listMarketBook', params).call(session)
    if response:
        if not apierrorhandling.api_betting_error_handling(response, params):
            return
        if parsed:
...
def place_orders(api, params, session=None, parsed=True):  # atomic
    (response, raw_response, sent) = BettingRequest(api, 'SportsAPING/v1.0/placeOrders', params).call(session)
    if response:
...
def cancel_orders(api, params=None, session=None, parsed=True):
    ...
    (response, raw_response, sent) = BettingRequest(api, 'SportsAPING/v1.0/cancelOrders', params).call(session)
    if response:
        date_time_received = datetime.datetime.now()
        apierrorhandling.api_order_error_handling(response)
        if 'error' in response:
...
def update_orders(api, params, session=None, parsed=True):
    (response, raw_response, sent) = BettingRequest(api, 'SportsAPING/v1.0/updateOrders', params).call(session)
    if response:
        date_time_received = datetime.datetime.now()
        apierrorhandling.api_order_error_handling(response)
        if 'error' in response:
...
def replace_orders(api, params, session=None, parsed=True):
    (response, raw_response, sent) = BettingRequest(api, 'SportsAPING/v1.0/replaceOrders', params).call(session)
    if response:
        date_time_received = datetime.datetime.now()
        apierrorhandling.api_order_error_handling(response)
        if 'error' in response:
...
def list_current_orders(api, params=None, session=None, parsed=True):  # todo handle moreAvailable
    if not params:
        params = {'dateRange': {}}
    (response, raw_response, sent) = BettingRequest(api, 'SportsAPING/v1.0/listCurrentOrders', params).call(session)
    if response:
        if not apierrorhandling.api_betting_error_handling(response, params):
            return
        if parsed:
...
def list_cleared_orders(api, params=None, session=None, parsed=True):  # todo handle moreAvailable & groupby params
    if not params:
        params = {'betStatus': 'SETTLED',
                  'settledDateRange': {},
                  'recordCount': '1000'}
    (response, raw_response, sent) = BettingRequest(api, 'SportsAPING/v1.0/listClearedOrders', params).call(session)
    if response:
        if not apierrorhandling.api_betting_error_handling(response, params):
            return
        if parsed:
...
def list_market_profit_and_loss(api, params=None, session=None, parsed=True):
    ...
    (response, raw_response, sent) = BettingRequest(api, 'SportsAPING/v1.0/listMarketProfitAndLoss', params).call(session)
    if response:
        if not apierrorhandling.api_betting_error_handling(response, params):
            return
        if parsed:
...
def list_race_status(api, params, session=None, parsed=True):
    (response, raw_response, sent) = ScoresRequest(api, 'ScoresAPING/v1.0/listRaceDetails', params).call(session)
    if response:
        if not apierrorhandling.api_betting_error_handling(response, params):
            return
        if parsed:
...
def get_account_funds(api, params=None, session=None, parsed=True):
    ...
    (response, raw_response, sent) = AccountRequest(api, 'AccountAPING/v1.0/getAccountFunds', params).call(session)
    if response:
        if parsed:
...
def get_account_details(api, params=None, session=None, parsed=True):
    if not params:
        params = {}
    (response, raw_response, sent) = AccountRequest(api, 'AccountAPING/v1.0/getAccountDetails', params).call(session)
    if response:
        if parsed:
...
def get_account_statement(api, params=None, session=None, parsed=True):
    if not params:
        params = {'itemDateRange': {},
                  'includeItem': 'ALL'}
    (response, raw_response, sent) = AccountRequest(api, 'AccountAPING/v1.0/getAccountStatement', params).call(session)
    if response:
        if parsed:
...
def list_currency_rates(api, params=None, session=None, parsed=True):
    ...
    (response, raw_response, sent) = AccountRequest(api, 'AccountAPING/v1.0/listCurrencyRates', params).call(session)
    if response:
        if parsed:
...
def transfer_funds(api, params=None, session=None, parsed=True):
    if not params:
        params = {'from': 'UK',
                  'to': 'AUSTRALIAN',
                  'amount': '0.01'}
    (response, raw_response, sent) = AccountRequest(api, 'AccountAPING/v1.0/transferFunds', params).call(session)
    if response:
        if parsed:
